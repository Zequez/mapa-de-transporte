...
(function() {
  ...
  MapTools.Segment = (function() {
    ...
    Segment.prototype.closest_point = function(p) {
      var a, ab, ab2, ab_m_t, ap, ap_ab, b, point, t;
      a = this.p1;
      b = this.p2;
      ap = [p[0] - a[0], p[1] - a[1]];
      ab = [b[0] - a[0], b[1] - a[1]];
  ...
  BusEditor.Route.AddressesManager.Fetcher = (function() {
    ...
    Fetcher.prototype.build_queries = function() {
      if (this.address.number) {
        return this.queries.push(this.address.full_address);
      } else {
        this.queries.push("" + this.address.address + " " + this.start_number);
        return this.queries.push("" + this.address.address + " " + this.end_number);
      }
    };

    Fetcher.prototype.fetch = function(callback) {
      var query, _i, _len, _ref, _results;
      if (this.results.length === 0) {
        this.build_queries();
        this.queries_finished = 0;
        this.queries_results = [];
        _ref = this.queries;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          query = _ref[_i];
          _results.push(this.fetch_query(query, callback));
        }
        return _results;
      } else {
    ...
    Fetcher.prototype.fetch_query = function(query, callback) {
      var _this = this;
      return this.city.fetch_address(query, function(result) {
        if (result) {
          _this.results.push(result);
          ++_this.queries_finished;
          if (_this.queries_finished === _this.queries.length) {
